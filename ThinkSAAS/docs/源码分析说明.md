...
# 模板语法
官网没有详细的说明，只能从源码中分析，模板语法不外乎那几种形式，从html文件里可以找出来。    
再有的就是从`template()`源码分析，那里应该有相关解析的原理。    

```php
// 执行PHP代码
{php doAction('tseditor')} // 添加钩子
{php include pubTemplate("header")}
{php include pubTemplate("footer")} // 包含文件
...
然后不生效，是因为模块是自定义的模块，需要手动修改一下其返回值  
```php
// data\custom_plugins.php
return array (
    0 => 'tips',
);
```
...
```php
// index.php?app=system&ac=plugin&ts=do&apps=custom&pname=tips&isused=1
...
/**
 * 写入文件，支持Memcache缓存
 * @param string known $file 缓存文件
 * @param string known $dir 缓存目录
 * @param mixed own $data	缓存内容
 * @param number $isphp
 * @return boolean
 */
function fileWrite(string $file, string $dir, mixed $data, $isphp = 1) {
